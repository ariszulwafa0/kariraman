...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Kirim buffer ke fungsi analisis panggilan
        const result = await gemini.analyzeInvitation(buffer, true); // true menandakan ini gambar

        if (result) { res.status(200).json(result); } 
        else { res.status(500).json({ error: 'Gagal mendapatkan analisis dari AI.' }); }
    } catch (error) {
        console.error('API Error in cek-panggilan-foto:', error);
